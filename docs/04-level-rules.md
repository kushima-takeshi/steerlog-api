...
SectionStudyStatus更新後、対象Resourceの全Sectionが `studiedAt IS NOT NULL` になった場合、Lv.1到達候補にする。
...
3. Progress.lastStudiedAt = now
4. 全SectionのstudiedAtを確認
5. 全Section学習済みならProgress.currentLevelを1以上にする
6. LevelHistory Lv.1 がなければ作成
```
...
sourceId = 最後に更新したsectionStudyStatusId
